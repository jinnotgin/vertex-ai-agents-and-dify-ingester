...
async function crawlJql(
	...
	try {
		// New getIssuesByJQL returns all issues at once, handling pagination internally
		const issues = await getIssuesByJQL(jql, customFieldsMap);
		
		logger.info(
			`crawlJql: Found ${issues.length} issues total for query: ${jql}`
		);
		...
		for (const issue of issues) {
			...
			// Optional: Add small delay between processing issues
			// await sleep(DELAY_TIME);
		}

		logger.info(`crawlJql: ${functionName} - Completed processing all ${issues.length} issues for ${jql}`);
	} catch (error) {
		logger.error(`crawlJql: Error: ${error.message}`);
