...
function extractLinks(htmlContent, baseUrl, regex) {
	const dom = new JSDOM(htmlContent);
	const links = Array.from(dom.window.document.querySelectorAll('a[href]'))
		.map(a => {
			console.log(a.href)
			// console.log(new URL(a.href, baseUrl).href)
...
async function _crawlUrl(crawlTargetName, url, includeLinks = true, regex, depth = 1, headers = {}) {
	console.log("Url:", url)
	if (depth <= 0) return [];
	...
	const links = extractLinks(pageContent, url, regex);
	console.log({ regex, links })
	logger.info(`Found ${links.length} links on ${url}`);

	const nestedResults = [];
	if (includeLinks && links.length > 0) {
		for (const link of links) {
			const nestedResult = await _crawlUrl(crawlTargetName, new URL(link, url).href, includeLinks, regex, depth - 1, headers);
...
async function crawlUrl(crawlTargetName, url, includeLinks = true, regex, depth = 1, headers = {}) {
	return await retryAsyncFunction(_crawlUrl, RETRY_ASYNC_FUNCTION_SETTINGS, crawlTargetName, url, includeLinks, regex, depth, headers);
}
