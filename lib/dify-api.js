...
async function _difyPurgeDataStore(datasetId) {
	...
	let page = 1;
	let documentIds = new Set(); // Use a Set to avoid duplicate document IDs

	// Fetch all documents with pagination
	while (true) {
		logger.info(`Fetching document IDs for dataset ${datasetId} - Page ${page}`);
		...
		// Add new document IDs to the Set
		responseData.data.forEach(doc => documentIds.add(doc.id));

		logger.info(`Fetched ${documentIds.size} unique document IDs so far`);
	...
	logger.info(`Total unique documents to delete: ${documentIds.size}`);

	// Proceed with deletion of all documents
	for (const documentId of documentIds) {
		const deleteUrl = `${DIFY_BASE_URL}/datasets/${datasetId}/documents/${documentId}`;
		...
		if (!deleteResponse.ok) {
			...
			throw new Error(`Error deleting document ${documentId}: ${deleteError}`);
		}
		logger.info(`Deleted document ${documentId} from ${datasetId}`);
