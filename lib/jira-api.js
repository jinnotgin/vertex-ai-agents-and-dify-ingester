...
// Helper function for token-based pagination (new JQL search API)
async function postRequestWithTokenPagination(
	url,
	basePayload,
	resultsKey = "issues"
) {
	logger.info("postRequestWithTokenPagination: Fetching", { url, basePayload });
	...
	const fetchPage = async (pageToken = null) => {
		...
		if (!response.ok) {
			...
			logger.error(`postRequestWithTokenPagination: HTTP Error: ${response.status}`, { errorText });
			throw new Error(`HTTP Error: ${response.status} - ${errorText}`);
		}

		const json = await response.json();
		const results = json?.[resultsKey] || [];
		logger.info(`postRequestWithTokenPagination: ${results.length} results found on this page.`);

		const hasNextPage = !(json.isLast === true);
		let nextResults = [];

		if (hasNextPage && json.nextPageToken) {
			logger.info("More results available", { nextPageToken: json.nextPageToken });
			nextResults = await fetchPage(json.nextPageToken);
		}
...
*/
// This is a new helper function to handle the new token-based pagination recursively
async function _getIssuesByJQL(
	jql,
	customFieldsMap,
	fieldsByKeys = false,
	...
	logger.info("Fetching issues with JQL", { jql, pageToken });

	const payload = {
		jql,
		maxResults: 50, // Or make this configurable
		expand: "renderedFields", // Spec indicates string, not array
		fieldsByKeys, // New parameter from the spec
		fields: [
			"issuetype",
			"project",
			"labels",
			...
			"updated",
			"attachment",
			// If fieldsByKeys is true, you can use keys here instead of custom field IDs
			...Object.values(customFieldsMap),
		],
	};
	...
	try {
		const response = await fetch(buildUrl(JIRA_JQL_SEARCH_ENDPOINT), { // Ensure this endpoint is /rest/api/3/search/jql
			method: "POST",
			headers: BASE_HEADERS,
			body: JSON.stringify(payload),
		});
		...
		if (!response.ok) {
			const errorText = await response.text();
			logger.error(`_getIssuesByJQL: HTTP Error: ${response.status}`, { errorText });
			throw new Error(`HTTP Error: ${response.status} - ${errorText}`);
		}
		...
		// --- Data Formatting Logic (copied from your original function) ---
		const formattedIssues = issues.map((issue) => {
			const fields = issue.fields;
			const customFields = {};
			for (const [fieldName, fieldKey] of Object.entries(customFieldsMap)) {
				// If fieldsByKeys is true, the key in the response will be the field name/key, not the ID
				const responseKey = fieldsByKeys ? fieldName : fieldKey;
				customFields[fieldName] = fields[responseKey];
			}

			logger.info(`_getIssuesByJQL: Prepared formatted data for ${issue.key}`);
		...
		// --- End of Data Formatting Logic ---

		const hasNextPage = !(json.isLast === true);
		let nextResults = [];

		if (hasNextPage && json.nextPageToken) {
			logger.info("More issues available, fetching next page.", { nextPageToken: json.nextPageToken });
			// Recursive call to fetch the next page
			nextResults = await _getIssuesByJQL(
				jql,
				customFieldsMap,
				fieldsByKeys,
				json.nextPageToken
		...
		return [...formattedIssues, ...nextResults];

	} catch (error) {
		logger.error(`Error in _getIssuesByJQL: ${error.message}`);
		throw error;
	}
}
// This function will now retrieve ALL issues matching the JQL, handling all pagination internally.
async function getIssuesByJQL(jql, customFieldsMap, options = {}) {
    const { fieldsByKeys = false } = options;
	...
	return await retryAsyncFunction(
		...
		customFieldsMap,
			fieldsByKeys,
		null // Initial pageToken is null
	);
}

async function _getAllEpics(projectKey) {
	try {
		const results = await postRequestWithTokenPagination(
